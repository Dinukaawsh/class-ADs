      ...
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted">
            {institutes.length} verified institute{institutes.length !== 1 ? "s" : ""} found
            {q && ` for "${q}"`}
            {subject && ` · ${subject}`}
            {district && ` · ${district}`}
          </p>
        ...
        <div className="flex gap-2">
          <Link
            ...
            className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-foreground transition hover:bg-surface-alt"
          >
            Institute Dashboard
          </Link>
          <Link
      ...
      {institutes.length === 0 ? (
        <div className="rounded-2xl border-2 border-dashed border-border px-6 py-16 text-center">
          ...
          <p className="mt-2 text-sm text-muted">
            Try different filters, or create a new institute profile.
          </p>
          <Link
            href="/institutes/new"
